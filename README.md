# Claude & Agent Skills Collection

이 리포지토리는 AI 에이전트 및 LLM을 위한 커스텀 스킬(Skills) 모음입니다.

## 📄 등록된 스킬 목록 (Skills)

### 1. Docu-Git (`docu-git`)
- **설명**: Git이 소스코드를 관리하듯, 사용자의 문서 작업 요청(Input)과 산출물(Output)을 요약·기록하고 저장 시 파일명에 버전을 반영하는 문서 이력관리 에이전트 스킬
- **위치**: [`skills/docu-git`](./docu-git) 또는 [`docu-git/SKILL.md`](./docu-git/SKILL.md)
- **주요 기능**:
  - 사용자 Input/Output 연속 이력 축적 (`DOCU_HISTORY.md`)
  - 로컬 저장 파일명 버전 포함 규칙 적용 (`{문서명}_v{버전}_{주요변경사항}.{ext}`)
  - 핵심요지 손실 없는 고밀도 요약
  - Status / Log / Diff / Branch / Tag 문서 이력 관리 지원

### 2. HWPX Report Brief (`hwpx-report-brief`)
- **설명**: 한컴 HWPX 템플릿의 '스타일'(□·ㅇ·-·※·* 개요, 본문, 표/그림 제목 등)을 그대로 물려받아 한글(.hwpx) 공공기관 보고서를 생성하는 스킬. 파이썬으로 XML을 직접 생성하므로 한글 프로그램 없이 동작한다.
- **위치**: [`hwpx-report-brief`](./hwpx-report-brief) 또는 [`hwpx-report-brief/SKILL.md`](./hwpx-report-brief/SKILL.md)
- **주요 기능**:
  - 번들 템플릿(`assets/template.hwpx`, 지방공기업평가원 기본 보고서 양식) 스타일 상속 — 한글에서 스타일만 고쳐 저장하면 다음 생성부터 반영
  - 개요 스타일을 **자동 글머리표 문자**로 매칭(스타일 이름·ID가 바뀌어도 안전)
  - `.docx`/`.txt`/`.md` 원고 → 표지·목차·장 번호박스·표·그림까지 조판
  - 생성물 구조 검증기 동봉 (`references/verify.py`)

### 3. HWP Edit (`hwp-edit`)
- **설명**: 이미 존재하는 한글 문서(.hwp/.hwpx)를 열어 텍스트를 읽고 수정하는 스킬. 특히 **표로 된 양식의 빈 칸을 좌표로 정확히 채운다**(심사서·평가표·신청서·점검표). 내부 XML을 직접 파싱하므로 한글 프로그램이나 COM 없이 동작하고 원본 서식은 보존된다. 한글의 기본 저장 형식인 **바이너리 .hwp는 COM으로 .hwpx 변환 후 동일하게 처리**한다.
- **위치**: [`hwp-edit`](./hwp-edit) 또는 [`hwp-edit/SKILL.md`](./hwp-edit/SKILL.md)
- **주요 기능**:
  - 텍스트 추출·검색·치환, 문서 구조 분석
  - `dump_tables()`로 표의 행/셀 좌표를 먼저 확인한 뒤 `set_cell_text()`로 기입
  - `expect_label`로 기입 직전 같은 행의 라벨을 대조해 **좌표 착오를 차단**
  - 문단 리스트로 줄바꿈 처리 + 레이아웃 캐시 제거로 **셀 밖 넘침 방지**
  - 수정 전 자동 백업
  - **포맷 자동 판별**(`detect_format`) — 확장자가 아닌 파일 시그니처로 ZIP/OLE 구분
  - **COM 폴백**(`scripts/hwp_com.py`) — 바이너리 `.hwp` ↔ `.hwpx` 변환, PDF 내보내기
  - 실행은 `pip`가 아닌 `uv run --with` (uv 관리 환경에서 `pip install`은 실패)
- **`hwpx-report-brief`와의 구분**: 이 스킬은 **기존 문서 편집**, `hwpx-report-brief`는 **새 보고서 생성**이다.

### 4. Book Translation (`book-translation`)
- **설명**: 외국어 원서 PDF를 챕터별로 분리해 한국어로 **완역**하고, 작품마다 서지 정보·등장인물·심층 해설을 붙여 마크다운으로 남기는 스킬. 요약본이 아닌 전편 완역이 목표일 때 쓴다. 실제 완역 프로젝트(단편집 2종, 원문 약 39만 단어)에서 뽑아낸 절차다.
- **위치**: [`book-translation`](./book-translation) 또는 [`book-translation/SKILL.md`](./book-translation/SKILL.md)
- **주요 기능**:
  - PDF → 내장 TOC 확인 → 챕터별 원문 `.txt` 덤프까지 3단 파이프라인 (`scripts/`)
  - **축약 자동 검출** (`scripts/verify.py`) — 한글 글자수 ÷ 원문 단어수 비율에 문장 수·인용부호 수 대조를 더해 판정. 완역본은 오탐하지 않으면서, 대화가 증발한 축약본은 ratio 1.5~1.7 / quote 1~30%의 지문으로 걸러낸다
  - 작품 파일·색인 README 템플릿과 심층 해설 작성 기준 (`references/format.md`)
  - 완역 원칙, 문체·강조·인용 표기, 역주를 붙일 곳과 붙이지 않을 곳 (`references/style.md`)
  - 번역문을 스크립트로 조립하다 축약본을 낳은 실패 사례를 절차에 반영 — 원문은 읽고, 마크다운은 직접 쓴다

### 5. Humanizer (`humanizer`)
- **설명**: AI가 쓴 티가 나는 문장을 사람이 쓴 것처럼 고쳐 쓰는 스킬. **내용은 바꾸지 않고 문체만 손본다.** 위키백과의 "Signs of AI writing" 문서를 토대로 32개 패턴을 목록화했다. 원본은 [blader/humanizer](https://github.com/blader/humanizer) (MIT), 이 리포지토리에는 포크본을 옮겨 담았다.
- **위치**: [`humanizer`](./humanizer) 또는 [`humanizer/SKILL.md`](./humanizer/SKILL.md)
- **주요 기능**:
  - **내용 패턴** — 과장된 의의·유산 서술, 권위자 이름 나열, `-ing` 구문으로 때우는 얕은 분석, 홍보성 표현, 출처 불명의 "연구에 따르면", 판박이 전망·과제 단락
  - **어법 패턴** — AI 상투어, `is/are` 회피, "X가 아니라 Y다", 억지 3단 나열, 문장 첫머리 반복, 가짜 "from X to Y" 범위, 수동태와 주어 실종
  - **스타일 패턴** — 줄표(—) 남용, 굵은 글씨 과다, 볼드 소제목 목록, 제목 Title Case, 이모지, 굽은 따옴표
  - **챗봇 흔적** — 답변에 남은 대화체, 지식 한계 고지, 과한 맞장구, 군더더기·과잉 완충어, 뻔한 마무리 문장
  - 원문의 목소리를 유지하는 기준(`## Match the writer's voice`)과 패턴별 **고치기 전/후 예문** 수록
  - 패키지 구조 검증기 동봉 (`scripts/validate-package.py`)

---

## 🛠️ 스킬 사용 방법

이 리포지토리의 스킬 폴더를 에이전트 커스텀 스킬 디렉토리(`.agents/skills/` 또는 사용자 지정 스킬 루트)에 배치하여 즉시 사용할 수 있습니다.
