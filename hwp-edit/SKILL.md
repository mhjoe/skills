...
description: >-
  ...
  양식에 내용을 작성해줘", "표의 해당 칸에 체크해줘", "이 한글 파일 pdf로 바꿔줘"
  같은 요청을 할 때. 파이썬으로 내부 XML을 직접 파싱하므로 한글 프로그램이나 COM
  없이 동작하고, 원본의 서식은 그대로 유지된다. 빈 문서에서 새 보고서를 생성하는
  작업이 아니라 기존 문서를 편집하는 작업에 쓴다(새 보고서 생성은
  hwpx-report-brief 스킬). 한글의 기본 저장 형식인 바이너리 .hwp는 COM으로
  .hwpx 변환 후 동일하게 처리한다.
---
...
- 문서를 PDF로 내보내야 할 때 (`to_pdf()` — XML로는 불가능해 COM을 쓰므로 한글 필요)
...
표를 넘어 본문 문단까지 직접 편집한다면 이어지는
[XML을 직접 편집할 때의 규칙](#xml을-직접-편집할-때의-규칙)도 함께 지킨다.
...
효과가 없을 뿐 아니라, **한글이 그 문서를 저장하면 `\n` 뒤의 텍스트가 사라진다**
(아래 **XML을 직접 편집할 때의 규칙 D**). 여러 줄은 리스트로 전달한다.

- ❌ `set_cell_text(4, 3, 1, ["1문단\n2문단"])` → `ValueError`로 차단됨
- ✅ `set_cell_text(4, 3, 1, ["1문단", "2문단"])`

또한 `<hp:linesegarray>`는 **렌더링된 줄 수만큼의 레이아웃 캐시**이고,
`<hp:lineseg>`의 `textpos`는 **문단 텍스트에 대한 문자 인덱스**다. 이걸 남긴 채
텍스트 길이를 바꾸면 길어졌을 때는 한 줄로 셀 밖에 넘치고, **짧아졌을 때는 파일이
아예 열리지 않는다**(아래 [XML을 직접 편집할 때의 규칙](#xml을-직접-편집할-때의-규칙)). `set_cell_text()`는 이 캐시를 제거한다.
...
4. 저장 후 `get_cell_text()`로 되읽어 검증한다. 단 이 메서드는 셀 안의
   `<hp:t>`를 **공백으로 이어붙이므로 문단 경계를 알려주지 않는다.** 여러 문단을
   넣었다면 문단 수까지 확인한다:

   ```python
   cell = doc.get_cell(0, 1, 1)
   n = len([e for e in cell.iter() if e.tag.endswith('}p')])   # 넣은 문단 수와 같아야 한다
   ```
5. 검증 결과를 사용자에게 보고한다
...
`set_cell_text()` 같은 제공 API를 벗어나 XML을 손으로 다루면, **파일이 아예 열리지
않거나 표가 통째로 사라지는** 실패가 생긴다. 아래 셋은 실제로 한 번씩 겪은 사고다.

### A. 문단 텍스트를 줄이면 `linesegarray`를 반드시 지운다 (안 지우면 파일이 안 열린다)

`<hp:lineseg>`의 `textpos`는 그 문단 텍스트의 문자 인덱스다. 런을 지우거나 더 짧은
문자열로 치환해 문단이 짧아졌는데 캐시를 남기면, `textpos`가 실제 길이를 넘어선다.
그 파일을 열면 한글이 이렇게 거부한다.

> 파일이 손상되었거나 다른 프로그램에 의해 변경되었습니다.

**XML은 완벽히 well-formed이고 ZIP도 정상이다.** `lxml` 파싱이나
`zipfile.testzip()`으로는 **절대 잡히지 않는다.** 실제 사례: 문단에서 272자짜리
런 하나를 제거했더니 남은 `lineseg` 30여 개가 범위를 벗어나 문서 전체가 열리지
않았다.

> **불변 규칙 — 문단의 텍스트 길이를 바꾸는 모든 편집은 그 문단의
> `<hp:linesegarray>`를 함께 제거한다.** 런 추가·삭제, `hp:t` 치환, 런 분할,
> 예외 없다. 캐시는 한글이 열 때 다시 계산하므로 **지워서 잃는 것은 없고,
> 남겨서 잃는 것은 파일 전체다.**

```python
NS = '{http://www.hancom.co.kr/hwpml/2011/paragraph}'
def drop_linesegs(paragraph):
    for lsa in paragraph.findall(NS + 'linesegarray'):
        paragraph.remove(lsa)
```

`set_cell_text()`·`replace_text()`·`replace_first()`는 이미 내부에서 처리한다.
**직접 XML을 다룰 때가 위험하다.**

### B. 저장 직후 한글이 실제로 여는지 확인한다

위와 같은 손상은 정적 검사로 잡히지 않으므로, **한글에 실제로 열려 보는 것이
유일한 검증**이다.

```python
from hwp_com import can_open
assert can_open("수정본.hwpx"), "한글이 열지 못한다 — linesegarray를 의심할 것"
```

편집을 여러 단계로 나눠 했다면 단계마다 호출해 **어느 편집이 깨뜨렸는지 이분
탐색으로 좁힌다.** 실제로 이 방법으로 원인을 5분 만에 특정했다.

**다만 `can_open()`은 "열리는가"만 본다.** 열리지만 한글이 **저장할 때 내용을 버리는**
상태가 따로 있다(아래 **D**). XML을 직접 만들어 넣었다면 왕복까지 확인한다.

```python
from hwp_com import roundtrip
rt = roundtrip("수정본.hwpx")
assert HwpDocument("수정본.hwpx").get_text() == HwpDocument(rt).get_text()
```

### C. 문단을 통째로 갈아끼울 때 개체를 삭제하지 않는다

본문 문단에는 텍스트만 있지 않다. **표·그림이 같은 `<hp:p>` 안에, 심지어 같은
`<hp:run>` 안에 텍스트와 섞여 있다.**

```xml
<hp:p><hp:run charPrIDRef="30">
  <hp:t>앞 문장</hp:t><hp:tbl .../><hp:t>뒤 문장</hp:t>
</hp:run></hp:p>
```

따라서 "첫 런만 남기고 나머지를 지운 뒤 텍스트를 새로 넣는" 방식은 **표를 통째로
삭제한다.** 실제로 이 실수로 표 2개가 사라졌다. 문단을 갈아끼우는 함수에는 가드를
둔다.

```python
def has_object(run):
    return run.find(NS+'tbl') is not None or run.find(NS+'pic') is not None

if any(has_object(r) for r in p.findall(NS+'run')):
    raise RuntimeError('문단에 표/그림이 있다 — 텍스트만 부분 치환할 것')
```

같은 이유로 **텍스트를 추출할 때 개체가 든 런을 통째로 건너뛰면 본문을 놓친다.**
`hp:tbl` 하위만 가지치기하고 `hp:t`는 모두 모아야 한다.

### D. `<hp:t>` 안에 개행문자를 넣지 않는다 (넣으면 한글이 저장할 때 뒤 텍스트를 버린다)

줄바꿈은 `<hp:p>` 경계이므로 `<hp:t>` 안의 개행은 화면에 나타나지 않는다 — **여기까지는
표시 문제지만, 진짜 피해는 그다음이다.** 그 문서를 한글에서 한 번 열어 저장하면
**개행 뒤의 텍스트가 통째로 사라진다.**

| 만든 값 (사이에 개행) | 한글이 저장한 뒤 |
|---|---|
| `0.1649*** (0.0087)` | `0.1649***` |
| `F(10, 39965)=39.54***` | `F(10, 39965)` |
| `조절계수 (X×Z, X→M)` | `조절계수` |

실제 사례: 회귀표의 표준오차 44칸과 표 머리글이 이렇게 날아갔다. **파일은 정상적으로
열리므로 `can_open()`으로도 잡히지 않고**, 사용자가 한글에서 저장하기 전까지는 아무
이상이 없다가 나중에 드러난다. 배포한 파일의 크기가 커져 있다면 한글이 다시 저장했다는
신호다.

셀 안에서 줄을 나눠야 한다면 **문단을 나누거나**(`set_cell_text`에 리스트 전달),
**한 줄로 합친다**(`0.1649*** (0.0087)`처럼 공백으로). 전자가 원래 조판에 가깝고
후자가 단순하다.

```python
# 저장 직전 정적 점검 - 싸고 확실하다 (COM 불필요)
bad = doc.find_text_newlines()
assert not bad, f"hp:t 안의 개행 {len(bad)}곳 - 한글 저장 시 유실된다"
```

`set_cell_text()`는 개행이 든 문자열을 `ValueError`로 막는다. **직접 XML을 만들 때가
위험하다.**
...
| 함수 | 설명 |
|---|---|
| `find_text_newlines()` | `<hp:t>` 안의 개행문자를 찾는다 — **한글 저장 시 유실되는 지점.** 저장 직전 정적 점검 |
| `hwp_com.can_open(path)` | **한글이 실제로 여는지 확인** — XML 파싱·ZIP 검사로는 잡히지 않는 손상을 잡는다. 저장 직후 호출할 것 |
| `hwp_com.roundtrip(path)` | 한글로 열어 다시 저장한 사본을 만든다 — 저장 과정의 내용 유실 확인 |
...
### 표의 일부 내용이 사라졌다 (표준오차·괄호·머리글 등)

**`<hp:t>` 안에 개행문자를 넣은 경우다.** 편집 직후에는 멀쩡하고 `can_open()`도
통과하지만, 한글에서 한 번 열어 저장하는 순간 개행 뒤 텍스트가 사라진다. 사용자가
파일을 열어본 뒤에야 드러나므로 원인을 찾기 어렵다.

1. `doc.find_text_newlines()`로 위험 지점을 찾는다 (정적, COM 불필요).
2. 해당 값을 **문단 분리** 또는 **한 줄 합치기**로 바꾼다 (위 **규칙 D**).
3. `hwp_com.roundtrip()`으로 왕복시켜 텍스트가 보존되는지 확인한다.

배포한 파일의 크기가 커져 있다면 한글이 다시 저장했다는 신호다.

### `파일이 손상되었거나 다른 프로그램에 의해 변경되었습니다` — 한글이 열지 못함

**XML을 직접 편집한 뒤 문단의 `<hp:linesegarray>`를 남겨둔 경우다.** 편집으로 문단
텍스트가 짧아지면 `lineseg`의 `textpos`가 실제 길이를 넘어서고, 한글은 이를 손상으로
판단한다. ZIP도 XML도 멀쩡하므로 `testzip()`·`lxml` 파싱으로는 잡히지 않는다.

진단과 복구:

1. `can_open()`으로 확인한다 — 정적 검사는 이 손상을 통과시킨다.

   ```python
   from hwp_com import can_open
   print(can_open("수정본.hwpx"))   # False면 이 항목이다
   ```

2. 정상본에서 다시 시작해 **편집을 한 단계씩 적용하며 매번 `can_open()`** 으로
   어느 편집이 깨뜨렸는지 좁힌다.
3. 해당 편집에 `drop_linesegs()`를 추가한다 (위 **XML을 직접 편집할 때의 규칙 A**).

`replace_text()`가 **더 짧은 문자열로** 치환하는 경우가 가장 흔한 함정이다.
스킬의 핸들러는 이미 캐시를 제거하지만, 직접 짠 XML 코드에는 빠지기 쉽다.
